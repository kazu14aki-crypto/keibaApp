...
export const FACTOR_DEFS = [
  ...
  {
    key: 'training', label: '調教評価', max: 10, hint: '追い切りタイム評価(解析パック取込で自動入力・手動調整可)',
    auto: false,
    guide: [
      'predict_today.py --json で出力した解析パックを取り込むと、ウッド・坂路の追い切りタイムから自動採点される(5点=平均)',
      '自動採点の根拠は「終い1Fの速さ」「自己平常時計との比較(調子の上向き)」「乗り込み本数」',
      '併せ馬の内容・馬なりか一杯かなど、タイムに表れない情報があれば手動で加減点を',
      'パック未取込の場合は中立の5点のままでよい',
    ],
  },
  {
  ...
  {
    key: 'sameCourse', label: '同コース実績', max: 20, hint: '今回と同じ競馬場・芝ダートでの過去成績から自動計算',
    auto: true,
    guide: [
      '今回と同じ競馬場・芝/ダートの組み合わせでの着順を評価（G1/G2は重み付き）',
      '着差が小さければ大敗でも内容として評価（0.3秒差10着は大幅減点しない）',
      '初出走の場合はやや中立寄りの値になる（7点）',
      'コース替わりで変身した馬を見つけるには手動で調整するとよい',
    ],
  },
];
